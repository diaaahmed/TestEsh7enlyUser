package com.esh7enly.data.url;

public class Urls {

    public static final String LOGIN = "auth/login";
    public static final String UPDATE_PASSWORD = "misc/change-password";
    public static final String UPDATE_PROFILE = "misc/update-profile";
    public static final String GET_DEPOSITS = "payment-method/list";
    public static final String FORGET_PASSWORD = "auth/send-forget-password-otp";
    public static final String NEW_PASSWORD = "auth/forget-password";

    public static final String OTP = "auth/otp";
    public static final String REGISTER = "auth/register";
    public static final String SEND_OTP = "auth/send-otp";
    public static final String SCHEDULE_INVOICE = "schedule/add";
    public static final String SCHEDULE_INQUIRE = "schedule/check";
    public static final String SCHEDULE_LIST = "schedule/list";
    public static final String IMAGE_ADS = "news/all";
    public static final String USER_POINTS = "points/get-points";
    public static final String REPLACE_POINTS = "points/redeem-points";

    public static final String SERVICES = "service/list";

    public static final String WALLETS = "wallet/all";

    public static final String TRANSACTIONS = "service/transactions";

    public static final String TRANSACTION_DETAILS = "service/show-transaction/{TRANSACTION_ID}";


    public static final String INQUIRY = "service/inquiry";

    public static final String PAYMENT = "service/payment";

    public static final String TOTAL_AMOUNT = "service/total-amount";
    public static final String CHARGE_BALANCE = "payment-method/pay-amount";
    public static final String START_SESSION = "payment-method/start-session";

    public static final String TOTAL_XPAY = "payment-method/get-amount";


   public static final String CANCEL_SERVICE = "service/cancel";

   public static final String CHECK_INTEGRATION_PROVIDER_STATUS = "service/check-integration-provider-status";

}