...
class RegisterFragment : BaseFragment<FragmentRegisterBinding, CreateAccountViewModel>() {
    ...
    private fun createAccount()
    {
        ...
        if(connectivity?.isConnected == true)
        {
            ...
            {
                dialog.showWarningDialog(resources.getString(R.string.error_message__blank_first_name),
                    resources.getString(R.string.app__ok))
                dialog.show()
            }
            else if(lastName.isEmpty())
            {
                dialog.showWarningDialog(resources.getString(R.string.error_message__blank_last_name),
                    resources.getString(R.string.app__ok))
                dialog.show()
            }
            else if(password.isEmpty() || confirmPassword.isEmpty())
            {
                dialog.showWarningDialog(resources.getString(R.string.error_message__blank_password),
                    resources.getString(R.string.app__ok))
                dialog.show()
            }
            else if(password != confirmPassword)
            {
                ...
                dialog.showWarningDialog(resources.getString(R.string.password_not_match_confirm_password),
                    resources.getString(R.string.app__ok))
                dialog.show()
            }
            ...
            {
                dialog.showWarningDialog(getString(R.string.password_is_not_valid_please_use_aa1),
                    resources.getString(R.string.app__ok))
                dialog.show()
            }
            else if(!isValidEmailId(email))
            {
                dialog.showWarningDialog(getString(R.string.email_is_not_valid),
                    resources.getString(R.string.app__ok))
                dialog.show()
            }
            else
